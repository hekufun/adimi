...
viewport.addEventListener("touchmove", (e) => {
    ...
    // Один палец — перемещение (1:1, без ускорения — на тач это ощущается точнее)
    if (e.touches.length === 1) {
    ...
    if (e.touches.length === 2) {
        ...
        let newScale =
            oldScale * (distance / (lastTouchDistance || distance || 1));
...
window.addEventListener("resize", () => {
    ...
    // На повороте экрана/смене размеров окна — переподгоняем сцену заново,
    // чтобы полка не оставалась обрезанной или неудобно смещённой.
